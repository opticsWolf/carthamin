...
**31 lexers ported**, **289 lexer tests**, **299 total tests** (all passing)::
...
Generated by ``generators/gen_lexers.py`` from Pygments source:
...
Covers: TypeScript, Ada, Haskell, OCaml, Nim, Zig, V, Vala, Perl, Ruby, PHP variants, Lua, JSON variants, YAML, TOML, XML, CSV, HTML variants, reStructuredText, INI, Apache, Nginx, Docker, Kubernetes, SQL variants, MongoDB, Terraform, Ansible, Brainfuck, Whitespace, Angular2, Handlebars, Liquid, Mako, Smarty, Twig, Velocity, and more.

~88 lexers remaining (of ~598 in Pygments — 54 delegating with missing root/lang modules, 61 custom Lexer subclasses).
...
::
    ...
    │       ├── lexer/           # Lexer engine + 515 ported lexers
    │       │   ├── mod.rs       # Lexer trait, RegexLexer, LexerRule, DelegatingLexer
    │       │   ├── regex_lexer.rs
    │       │   ├── extended.rs  # ExtendedRegexLexer, bygroups, using, include, inherit, combined, default
    │       │   ├── erb.rs       # ERB template lexer (manual port)
    │       │   ├── python.rs, javascript.rs, ...
    │       │   └── ...          # 557 Rust files (29 manual + 484 auto-generated + 4 other)
    ...
    │   └── gen_lexers.py        # Pygments lexer → Rust generator
...
To regenerate all auto-generated lexers from Pygments source::

    python generators/gen_lexers.py

This generates 454 pure RegexLexer + 30 DelegatingLexer files (484 total) into ``carthamin-core/src/lexer/``.
...
| 6        | Core Formatters     | ✅    | ✅                   |
+----------+---------------------+-------+----------------------+
| 7        | Extra Formatters    | ⚠️ Partial | 8 of 13 done       |
+----------+---------------------+-------+----------------------+
| 8        | Critical Lexers     | ✅    | ✅ (293/293)         |
+----------+---------------------+-------+----------------------+
| 9        | Lexer Code Gen      | ✅    | ✅ (480 generated)   |
+----------+---------------------+-------+----------------------+
| 10       | Registry & Public   | ✅    | ✅                   |
|          | API                 |       |                      |
+----------+---------------------+-------+----------------------+
...
+----------+---------------------+-------+----------------------+
| 12       | Remaining Lexers    | ⚠️ Partial | 515 total          |
+----------+---------------------+-------+----------------------+
| 13       | Final Polish        | ⬜    |                      |
+----------+---------------------+-------+----------------------+
