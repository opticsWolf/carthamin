...
/// Auto-generated apdlexer lexer.
pub struct apdlexerLexer {
    inner: RegexLexer,
}

impl apdlexerLexer {
    pub fn new() -> Self {
        let mut inner = RegexLexer::new("ANSYS parametric design language");
        inner.aliases = vec![
            "ansys",
            "apdl",
        ];
        ...
        inner.states.insert("core".to_string(), vec![
            ...
            LexerRule { pattern: TokenPattern::new(r"/EOF", Token::GENERIC_EMPH).unwrap(), action: LexerAction::token(Token::GENERIC_EMPH) },
        ...
        inner.states.insert("nums".to_string(), vec![
            LexerRule { pattern: TokenPattern::new(r"[+-]?\d*\.\d+([efEF][-+]?\d+)?", Token::NUMBER_FLOAT).unwrap(), action: LexerAction::token(Token::NUMBER_FLOAT) },
            LexerRule { pattern: TokenPattern::new(r"([+-]?\d+([efEF][-+]?\d+))", Token::NUMBER_FLOAT).unwrap(), action: LexerAction::token(Token::NUMBER_FLOAT) },
            LexerRule { pattern: TokenPattern::new(r"\b\d+(?![.ef])", Token::NUMBER_INTEGER).unwrap(), action: LexerAction::token(Token::NUMBER_INTEGER) },
        ]);
