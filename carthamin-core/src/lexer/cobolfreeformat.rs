...
/// Auto-generated CobolFreeformatLexer lexer.
pub struct CobolFreeformatLexer {
    inner: RegexLexer,
}

impl CobolFreeformatLexer {
    pub fn new() -> Self {
        let mut inner = RegexLexer::new("COBOLFree");
        inner.aliases = vec![
            "cobolfree",
        ];
        inner.filenames = vec![
        ...
        inner.states.insert("comment".to_string(), vec![
            LexerRule { pattern: TokenPattern::new(r"(\*>.*\n|^\w*\*.*$)", Token::COMMENT).unwrap(), action: LexerAction::token(Token::COMMENT) },
        ]);
        ...
        CobolFreeformatLexer { inner }
    }
}

impl Lexer for CobolFreeformatLexer {
