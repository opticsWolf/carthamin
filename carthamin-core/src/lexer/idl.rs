...
impl IDLLexer {
    pub fn new() -> Self {
        let mut inner = RegexLexer::new("IDL");
        inner.aliases = vec![
            "idl",
        ];
        inner.filenames = vec![
        ...
        let mut root_rules: Vec<LexerRule> = Vec::new();
            ...
            root_rules.push(LexerRule { pattern: TokenPattern::new(r"\b[+\-]?([0-9]*\.[0-9]+|[0-9]+\.[0-9]*)(D|E)?([+\-]?[0-9]+)?\b", Token::NUMBER_FLOAT).unwrap(), action: LexerAction::token(Token::NUMBER_FLOAT) });
            root_rules.push(LexerRule { pattern: TokenPattern::new(r"\b\'[+\-]?[0-9A-F]+\'X(U?(S?|L{1,2})|B)\b", Token::NUMBER_HEX).unwrap(), action: LexerAction::token(Token::NUMBER_HEX) });
            root_rules.push(LexerRule { pattern: TokenPattern::new(r"\b\'[+\-]?[0-7]+\'O(U?(S?|L{1,2})|B)\b", Token::NUMBER_OCT).unwrap(), action: LexerAction::token(Token::NUMBER_OCT) });
            root_rules.push(LexerRule { pattern: TokenPattern::new(r"\b[+\-]?[0-9]+U?L{1,2}\b", Token::NUMBER_INTEGER_LONG).unwrap(), action: LexerAction::token(Token::NUMBER_INTEGER_LONG) });
            root_rules.push(LexerRule { pattern: TokenPattern::new(r"\b[+\-]?[0-9]+U?S?\b", Token::NUMBER_INTEGER).unwrap(), action: LexerAction::token(Token::NUMBER_INTEGER) });
            root_rules.push(LexerRule { pattern: TokenPattern::new(r"\b[+\-]?[0-9]+B\b", Token::NUMBER).unwrap(), action: LexerAction::token(Token::NUMBER) });
